use super::{cross::cross_version, Arg, CommandDef};
use crate::{
  gen::{CodeGen, ToLit},
  Target, Version,
};
...
pub fn generate(def: Vec<(Version, CommandDef)>, target: Target, dir: &Path) -> io::Result<()> {
...
pub fn generate_ty(def: &CommandDef, target: Target) -> String {
  let mut gen = CodeGen::new();
  gen.write_line("/// Auto generated command argument kind. This is directly generated");
  gen.write_line("/// from bamboo data.");
  gen.write_line("#[derive(Debug, Clone, PartialEq)]");
  gen.write("pub enum Arg ");
  gen.write_block(|gen| {
    for b in &def.args {
  ...
  gen.write_block(|gen| {
    ...
    gen.write_block(|gen| {
      ...
      gen.write_block(|gen| {
        for (id, b) in def.args.iter().enumerate() {
          gen.write("Self::");
          gen.write(&b.name.to_case(Case::Pascal));
...
fn generate_versions(versions: &[(Version, CommandDef)]) -> String {
  let mut gen = CodeGen::new();

  gen.write_line("/// Generates the cross-versioning data for command arguments. This is how");
  gen.write_line("/// old clients get the same command nodes as newer ones.");
...
fn arg_data(gen: &mut CodeGen, b: &Arg) {
