use super::{cross::cross_version_metadata, Entity};
use crate::{gen::CodeGen, Version};

pub fn entity_metadata(gen: &mut CodeGen, entities: &[(&Version, Option<&Entity>)]) {
  gen.write("EntityMetadata ");
  gen.write_block(|gen| {
    let latest = &entities.last().unwrap().1.unwrap();

    gen.write_line("fields: &[");
    gen.add_indent();
    for meta in &latest.metadata {
      gen.write("MetadataField ");
      gen.write_block(|gen| {
        gen.write("id: ");
        gen.write(&meta.id.to_string());
        gen.write_line(",");
        gen.write("name: \"");
        gen.write(&meta.name);
        gen.write_line("\",");
        gen.write("ty: MetadataType::");
        gen.write(&format!("{:?}", meta.ty));
        gen.write_line(",");
      });
      gen.write_line(",");
    }
    gen.remove_indent();
    gen.write_line("]");

    gen.write_line("versions: &[");
    gen.add_indent();
    for (v, e) in entities {
      if let Some(e) = e {
        cross_version_metadata(gen, v, e, latest);
      }
    }
    gen.remove_indent();
    gen.write_line("]");
  });
}
