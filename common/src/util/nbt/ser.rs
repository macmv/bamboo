...
where
  T: Serialize,
{
  let mut serializer = Serializer { out: Buffer::new(vec![]) };
  value.serialize(&mut serializer)?;
  Ok(serializer.out.into_inner())
}
...
impl<'a> ser::Serializer for &'a mut Serializer {
  ...
  fn serialize_str(self, v: &str) -> Result<()> {
    self.out.write_i32(v.len() as i32);
    self.out.write_buf(v.as_bytes());
    Ok(())
  }
  fn serialize_bytes(self, v: &[u8]) -> Result<()> {
    self.out.write_i32(v.len() as i32);
    self.out.write_buf(v);
    Ok(())
  }

  // An absent optional is invalid.
  ...
  // There is no unit type in NBT data
  fn serialize_unit(self) -> Result<()> {
    Err(Error::NoUnitType)
  }
