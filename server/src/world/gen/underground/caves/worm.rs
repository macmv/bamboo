...
impl CaveWorm {
  pub fn new(seed: u64, pos: Pos) -> Self {
    ...
    let angle_y = ((rng.next_u32() % 1000) as f64 / 1000.0 - 0.5) * std::f64::consts::PI; // offset by +- PI
  ...
  pub fn carve(&mut self, chunk_pos: ChunkPos, c: &mut MultiChunk) {
    ...
    for _ in 0..steps {
      if self.pos.chunk() == chunk_pos {
        c.set_kind(self.pos.chunk_rel(), block::Kind::GreenWool).unwrap();
      }
      self.advance();
    }
  }
  ...
  fn advance(&mut self) {
    ...
    angle_y += ((self.rng.next_u32() % 1000) as f64 / 1000.0 - 0.5) * 0.8; // offset by +- 0.8
    self.direction.x = angle_y.cos();
    self.direction.z = angle_y.sin();
  }
}
