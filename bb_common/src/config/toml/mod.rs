...
impl From<String> for ValueInner {
  fn from(s: String) -> Self { ValueInner::String(s) }
}
...
fn is_valid_key(c: char) -> bool { c.is_ascii_alphabetic() || c == '-' || c == '_' }
  ...
  pub fn next(&mut self) -> Result<Token<'a>, ParseError> {
    ...
    loop {
      ...
      match c {
        ...
        c if c.map(|c| !is_valid_key(c)).unwrap_or(true) && found_word => {
          if let Some(c) = c {
            self.index -= 1;
            if c == '\n' {
              self.line -= 1;
        ...
        Some(c) if is_valid_key(c) => found_word = true,
...
impl Write for Writer {
  ...
  fn write_fmt(&mut self, s: std::fmt::Arguments) -> fmt::Result { self.s.write_fmt(s) }
}

impl Writer {
  pub fn new() -> Self { Writer { s: String::new(), path: vec![] } }
  pub fn write_table_start(&mut self) -> fmt::Result {
    write!(self, "\n[")?;
    for (i, segment) in self.path.iter().enumerate() {
      if i != 0 {
        write!(self.s, ".")?;
      }
      write!(self.s, "{segment}")?;
    }
    writeln!(self, "]")
  }
}
...
impl Value {
  pub fn to_toml(&self) -> String {
    ...
    self.write_toml(&mut w).unwrap();

    w.s
  }

  fn write_comments(&self, w: &mut Writer) -> fmt::Result {
    for line in &self.comments {
      writeln!(w, "# {line}")?;
    }
    Ok(())
  }
  fn write_toml(&self, w: &mut Writer) -> fmt::Result { self.value.write_toml(w) }
}
impl ValueInner {
  fn write_toml(&self, w: &mut Writer) -> fmt::Result {
    match self {
