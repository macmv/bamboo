...
fn test_index() {
  ...
  assert_eq!(s.index(SectionRelPos::new(0, 0, 0)), 0);
  assert_eq!(s.index(SectionRelPos::new(1, 0, 0)), 1);
  assert_eq!(s.index(SectionRelPos::new(2, 0, 0)), 2);
  assert_eq!(s.index(SectionRelPos::new(0, 0, 1)), 16);
  assert_eq!(s.index(SectionRelPos::new(15, 15, 15)), 4095);
}
#[test]
fn test_increase_bits_per_block() {
  unsafe {
    ...
    // Place some blocks
    s.set_palette(SectionRelPos::new(0, 0, 0), 0xf);
    s.set_palette(SectionRelPos::new(1, 0, 0), 0x0);
    s.set_palette(SectionRelPos::new(2, 0, 0), 0xf);
    s.set_palette(SectionRelPos::new(3, 0, 0), 0xa);
    // We want a split value
    s.set_palette(SectionRelPos::new(9, 0, 1), 0xf);

    s.data.increase_bpe(1);
    // Sanity check
    assert_eq!(s.data.bpe(), 5);
    // Get blocks should work
    assert_eq!(s.get_palette(SectionRelPos::new(0, 0, 0)), 0xf);
    assert_eq!(s.get_palette(SectionRelPos::new(1, 0, 0)), 0x0);
    assert_eq!(s.get_palette(SectionRelPos::new(2, 0, 0)), 0xf);
    assert_eq!(s.get_palette(SectionRelPos::new(3, 0, 0)), 0xa);
    assert_eq!(s.get_palette(SectionRelPos::new(9, 0, 1)), 0xf);
  }
}
#[test]
fn test_insert() {
...
fn test_shift_data_down() {
  ...
  // Removing 1 again undefined behavior, as 1 is in the block data now.
  // remove() should never be called with the given palette id present.
}
#[test]
fn test_set_get_block() {
  ...
  let mut s = Section::new(MAX_BPE);
  s.set_block(SectionRelPos::new(0, 0, 0), 5);
  ...
  s.set_block(SectionRelPos::new(1, 0, 0), 0);
  assert_eq!(s.block_amounts, vec![4095, 1]);
  assert_eq!(s.palette, vec![0, 5]);

  s.set_block(SectionRelPos::new(0, 0, 0), 0);
  assert_eq!(s.block_amounts, vec![4096]);
  assert_eq!(s.palette, vec![0]);

  // Make sure that higher palette ids get shifted down correctly.
  let mut s = Section::new(MAX_BPE);
  s.set_block(SectionRelPos::new(0, 0, 0), 10);
  assert_eq!(s.block_amounts, vec![4095, 1]);
  assert_eq!(s.palette, vec![0, 10]);
  ...
  assert_eq!(s.palette, vec![0, 5]);
  assert_eq!(s.block_amounts, vec![4095, 1]);
  s.set_block(SectionRelPos::new(1, 0, 0), 10);
  assert_eq!(s.palette, vec![0, 10]);
  assert_eq!(s.block_amounts, vec![4095, 1]);
  ...
  let mut s = Section::new(MAX_BPE);
  s.set_block(SectionRelPos::new(0, 0, 0), 10);
  assert_eq!(s.get_block(SectionRelPos::new(0, 0, 0)), 10);
  s.set_block(SectionRelPos::new(0, 0, 0), 123);
  dbg!(&s.reverse_palette);
  assert_eq!(s.get_block(SectionRelPos::new(0, 0, 0)), 123);
  s.set_block(SectionRelPos::new(1, 3, 2), 5);
  assert_eq!(s.get_block(SectionRelPos::new(1, 3, 2)), 5);
  s.set_block(SectionRelPos::new(15, 15, 15), 420);
...
fn test_set_all() {
  ...
  for x in 0..16 {
    for y in 0..16 {
      for z in 0..16 {
        s.set_block(SectionRelPos::new(x, y, z), 20);
      }
    }
  }
...
fn test_fill() {
  let mut s = Section::new(MAX_BPE);
  s.fill(SectionRelPos::new(0, 0, 0), SectionRelPos::new(15, 15, 15), 20);
  assert_eq!(s.data.clone_inner(), vec![0x1111111111111111; 16 * 16 * 16 * 4 / 64]);
  assert_eq!(s.palette, vec![0, 20]);
  assert_eq!(s.block_amounts, vec![0, 4096]);
  ...
  let mut s = Section::new(MAX_BPE);
  s.fill(SectionRelPos::new(3, 4, 5), SectionRelPos::new(8, 9, 10), 20);

  dbg!(&s);
  for x in 0..16 {
    for y in 0..16 {
      for z in 0..16 {
        let expected = if (3..=8).contains(&x) && (4..=9).contains(&y) && (5..=10).contains(&z) {
          20
        } else {
          0
        };
