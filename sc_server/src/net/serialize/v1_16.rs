...
// CHANGES:
// Biomes are now a varint array, instead of an int array.
pub fn serialize_chunk(pos: ChunkPos, c: &MultiChunk) -> cb::Packet {
  ...
  for s in c.sections().flatten() {
    ...
    if s.data().bpe() <= 8 {
      ...
      for g in s.palette() {
        chunk_data.write_varint(types.to_old(*g as u32, BlockVersion::V1_16) as i32);
      }
    }
  ...
  // Length is not present if full chunk is false
  if has_biomes {
  ...
  cb::Packet::MapChunk {
    ...
    biomes_v1_15:                          None,
    biomes_v1_16_2:                        Some(biomes.into_inner()),
